...
class _PortfolioScreenState extends State<PortfolioScreen> with TickerProviderStateMixin {
  ...
  void _initParticles(Size size) {
    ...
    int numGroups = 8;
    int totalStars = 1800; 
    int planetStarsCount = 1350; 
    
    double baseWidth = max(size.width, 1200.0);
    double baseHeight = max(size.height, 800.0);
    
    double saturnX = baseWidth * 0.72;
    double saturnY = baseHeight * 0.50;
    double bodyRadius = 138.0; 
    
    _particles = List.generate(totalStars, (index) {
      bool isPlanet = index < planetStarsCount;
      bool isRing = isPlanet && index >= 450; 
      ...
      if (isPlanet) {
        if (!isRing) {
          // CUERPO DE SATURNO (450 partículas en el BORDE)
          ...
          // ANILLOS DE SATURNO (900 partículas - ULTRA DENSIDAD)
          bool isInnerRing = index < 900; 
          ...
          pColor = isInnerRing ? const Color(0xFFFFF8DC) : const Color(0xFFEBC79E);
